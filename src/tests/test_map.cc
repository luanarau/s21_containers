...
TEST(map, MapAtOperatorException) {
  s21::map<char, std::string> my_map = {
      {'a', "Alina"}, {'b', "Boris"}, {'c', "Chuck"}};
  using typeException = s21::map<char, std::string>::Readwrite;
  EXPECT_THROW(my_map.at('g') = "Alisa", typeException);
}
...
TEST(map, InsMany) {
  s21::map<int, int> my_map;
  s21::vector vec = my_map.insert_many(
      std::make_pair(1, 10), std::make_pair(2, 20), std::make_pair(3, 30));
  auto x = (*(my_map.begin())).second;
  EXPECT_EQ(x, 10);
  s21::map<int, int> set({{1, 10},
                          {2, 20},
                          {3, 30},
                          {4, 40},
                          {5, 50},
